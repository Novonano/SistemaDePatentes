...
from pipeline.observers import PipelineEvent, StageEvent
...
class FrozenPipelineTests(unittest.TestCase):
    ...
    def test_run_agent_accepts_custom_observers(self):
        fixture_path = os.path.join("benchmarks", "frozen_pipeline_fixture.json")
        scrapers, evaluator_factory = build_frozen_components(fixture_path)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(state.status, "completed")
            self.assertTrue(captured, "nenhum evento capturado")
            ...
            stage_events = [e for e in captured if isinstance(e, StageEvent)]
            stages = {e.stage for e in stage_events}
            self.assertIn("setup", stages)
            self.assertIn("search", stages)
            self.assertIn("screening", stages)
